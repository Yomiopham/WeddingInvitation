...
const operations_1 = require("../../json-crdt-patch/operations");
...
class Model {
    ...
    applyOperation(op) {
        ...
        if (op instanceof operations_1.InsStrOp) {
        ...
        else if (op instanceof operations_1.InsVecOp) {
            const node = index.get(op.obj);
            const tuples = op.data;
            const length = tuples.length;
            if (node instanceof ArrayLww_1.ArrayLww) {
                for (let i = 0; i < length; i++) {
                    const tuple = tuples[i];
                    const valueNode = index.get(tuple[1]);
                    if (!valueNode)
        ...
        else if (op instanceof operations_1.DelOp) {
            const node = index.get(op.obj);
            if (node instanceof ArrayRga_1.ArrayRga) {
            ...
            if (!index.get(op.id))
                index.set(new ArrayLww_1.ArrayLww(this, op.id));
        }
    }
